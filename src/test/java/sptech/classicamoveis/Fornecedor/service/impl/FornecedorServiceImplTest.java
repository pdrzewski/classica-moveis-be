...
class FornecedorServiceImplTest {
 ...
 @Test void criaFornecedorComEndereco(){Endereco e=new Endereco();Fornecedor f=fornecedor(1);when(enderecoService.buscarEntidadePorId(3)).thenReturn(e);when(mapper.toEntity(dto(),e)).thenReturn(f);when(repository.save(f)).thenReturn(f);when(mapper.toResponseDTO(f)).thenReturn(resposta());assertEquals(1L,service.criar(dto()).id());}
